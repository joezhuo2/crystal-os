...
- [ ] v0.6.9 Home tooltips. Hover **Add task**, **Complete task**, the Archive's quick add and each Portal box app icon: every tooltip is drawn whole, over the neighbouring cards.
- [ ] v0.6.9 Vault charts. Open the Vault from another tab: Cash Flow, Spending Breakdown and Net Savings Trend show a skeleton briefly, then all three draw in together. With performance mode on they appear drawn, no entrance.
- [ ] v0.6.9 Upcoming order. With today clear, an urgent task next week sits above a low task tomorrow; tasks of equal priority run soonest first.
- [ ] v0.6.9 Archive box. The Archive card on Home shows the amethyst cave and sparkles; tag chips, notes, quick add and "Browse all" still work.
